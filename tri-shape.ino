...
MANUAL;
	A2 (CV in 1/knob 1): HS knob (keep switch in manual position, this acts as an offset for the HS parameter)
    Applies a mixture of hard-sync and phase distortion to the basic 
    triangle wave.
  A1: (CV in 2/knob 2): HS CV (keep switch in "in 2" position, knob acts as an attenuator for incoming HS CV)
...
void loop()
{
  ...
  wrap =  min( 64 + (min( 1023,  analogRead(WRAP_CV) + analogRead(WRAP_KNOB)) >> 1), 511);
  phase_dist_amount = min( 1023,  analogRead(HS_CV) + analogRead(HS_CONTROL)) >> 2;
}

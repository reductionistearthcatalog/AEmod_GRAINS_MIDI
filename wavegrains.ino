...
#define WAVETABLE_SELECT_KNOB   (A2)
#define WAVETABLE_SELECT_CV     (A1)
#define SWEEP_KNOB  (A0)
#define SWEEP_CV    (A3)
...
void setup() {
  ...
  // Initiate MIDI communications, listen to channel omni
  MIDI.begin(MIDI_CHANNEL_OMNI);
  // begin baud rate at 31250. not sure if this is necessary after MIDI.begin 
  Serial.begin(31250);
...
void loop() {
  ...
  sweepPosition = min( 1023,  analogRead(SWEEP_CV) + analogRead(SWEEP_KNOB));

  // Read current wavetable and set up wavetables to morph between
  uint8_t readWavetable = min( 1023,  analogRead(WAVETABLE_SELECT_CV) + analogRead(WAVETABLE_SELECT_KNOB)) >> WT_POT_SCALING;
